...
@pytest.mark.parametrize("g_bus_clk", [str(60)])
@pytest.mark.parametrize("g_sys_clk", [str(400),str(500)])
def test_uart(g_bus_clk,g_sys_clk):

    module = "testbench_i2c_master"
    toplevel = "f_i2c_master"   
    vhdl_sources = [
        os.path.join(rtl_dir, "f_i2c_master.vhd"),
        ]

    parameter = {}
    parameter['g_bus_clk'] = g_bus_clk
